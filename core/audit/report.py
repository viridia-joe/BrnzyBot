...
def _spec_from_log(spec_id, wow_class: str, stats: dict | None = None) -> str | None:
    ...
    TBC Anniversary), else infers role/spec from the stat block (the shared
    gear_cache.spec_from_stats heuristic), else the class default. Returns None
    only when even the class is unknown."""
    from core.gear_cache import WCL_SPEC_MAP, spec_from_stats
    if spec_id:  # 0 / None both mean "not provided"
        key = WCL_SPEC_MAP.get(int(spec_id)) if str(spec_id).lstrip("-").isdigit() else None
        if key:
            return key
    if stats:
        key = spec_from_stats(wow_class, stats)
        if key:
            return key
